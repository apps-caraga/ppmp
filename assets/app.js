     ...
     fetch(api_url + '/me', {
         ...
         .then(data => {
             if (data.id != null) {
                 ...
                 dcument.location.replace('login.php');
             }
         })
         .catch((error) => {
 ...
 function register(event) { // Define the login function
     event.preventDefault(); // Prevent the default form submission
     const new_username = document.getElementById('username').value; // Get username
     const new_password = document.getElementById('password').value; // Get password

     if (username.length === 0 || password.length === 0) {
         message('Please enter username and password.');
     } else {
         console.log(JSON.stringify({ username: new_username, password: new_password }));
         fetch(api_url + '/register', {
             ...
             .then(data => {
                 console.log(data)
                 if (data.id != null) {
                     message("Account created. Please contact your system admin for activation.".message, 'success');
                 } else {
     ...
     fetch(api_url + '/logout', {
         ...
         .then(data => {
             ...
             if (data.id != null) {
                 ...
                 document.location.replace('app.php');
             } else {
                 // Handle login failure (e.g., show an error message)
